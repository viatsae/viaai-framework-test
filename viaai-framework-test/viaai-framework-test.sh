...
if [[ "$NAME" == "CentOS Linux" ]]; then
  echo $NAME
  yum install -y python3-tk bsdmainutils less python3-matplotlib &> /dev/null
...
if [[ "$NAME" == "Ubuntu" ]]; then
  ...
  wget https://github.com/amplab/datascience-sp14/raw/master/lab7/mldata/mnist-original.mat &>/dev/null
  cd -
  skl_start=`date +%s`
  ...
  echo $NAME
  skl_start=`date +%s`
  python3 sklearn/sklearn_train_cent.py &> log/sklearn_result
  skl_end=`date +%s`
fi
